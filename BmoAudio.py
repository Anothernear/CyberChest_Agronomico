...
def Hablar(Texto, Modo):
    # INDICADOR DE INICIO
    with open("/tmp/bmo_speaking", "a") as f: pass
    
    Env = os.environ.copy()
    Env["LD_LIBRARY_PATH"] = PiperBinDir
    Texto = Texto.replace('"','').replace("'",'').replace('\n',' ').strip()
    if not Texto: 
        if os.path.exists("/tmp/bmo_speaking"): os.remove("/tmp/bmo_speaking")
        return
    enviar_emocion("hablar")
    ...
    # INDICADOR DE FIN
    if os.path.exists("/tmp/bmo_speaking"):
        os.remove("/tmp/bmo_speaking")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        Conexion.close()
        if DataAcumulada:
            Msg = DataAcumulada.decode("utf-8", errors="replace")
            ModoVoz, _, TextoFinal = Msg.partition("|")
